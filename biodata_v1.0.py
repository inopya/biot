...
##import re # lo usamos para verificar los email
...
#DATOS DEL SERVIDOR DE CORREO (cuenta de gmail) PARA REALIZAR LOS ENVIOS
#ojo, aqui estaran visibles la direccion y la contraseña :(
...
##SMTP_CORREO_ENVIOS = 'smtp-mail.outlook.com'
##EMAIL_CORREO_ENVIOS = 'direccion@hotmail.com'
##PASS_CORREO_ENVIOS = 'clave'
##REMITENTE_CORREO_ENVIOS = 'direccion@hotmail.com' #hotmail no permite otro remitente que no sea la direccion de salida


#CLAVE para la API de telegram
TOKEN = "984228214:AAFBx7QZTaAeyC9CAXGpvmyNg9K8OwDE4Ng" #IoTino_bio_bot (pruebas experimento bio)
...
ADMIN_USER = None  #numero ID de telegram
...
user_keyboard = [['/info','/fig'],['/send'],['/save','/ayuda']]
...
listaComandos = ["/ayuda - Mostrar esta Ayuda", \
                 "/send - pedir datos completos por email",\
...
lista_correo_experimento = ['sorbasdigital@hotmail.com']
...
################   " <---------- METADATOS -------------->   <------- SENSORES -------> "
cabeceraTXTdatos = "nº muestra\tESTACION\t\tFECHA\t\t\tHORA\t\t\tTemperatura\tHumedad\t\tPH\t\t\tCO2\t\t\tAzul\t\tRojo"
...
def atenderTelegramas(bot):
    ...
    global FLAG_enviar_PNG, FLAG_pruebas
        ...
        for update in bot.get_updates(offset=update_id, timeout=0): #timeout=5, si nos da problemas con internet lento
            ...
            if update.message:  # porque se podrian recibir updates sin mensaje...
                ...
                try:
                    ...
                    if comando == "/send" and chat_id == ADMIN_USER:
                    ...
                    if comando == "/save" and chat_id == ADMIN_USER:
                    ...
                    if comando == "/info":
                        send_message ("============================\n" +
                                      ...
                                      " Luz Azul:   " + str(valor_sensor_5_Now) + " \n" +
                                      " LUZ Roja: " + str(valor_sensor_6_Now)+" \n", chat_id)
                        return
...
def dibujar_grafica(datos_brutos_experimento):
    ...
    try:
        ...
        #etiqueta
        plt.title('Captura de datos Experimento BIO 2019, v1.0')
        fig.canvas.set_window_title('INOPYA')
        
        #representacion del dato3
        ##lw=5, c='y', marker='o', ms=10, mfc='red')
        plt.plot(puntos_eje_x, datos_por_sensor_y[0], lw=1.5, c='darkorange')  #temperatura
        plt.plot(puntos_eje_x, datos_por_sensor_y[1], lw=1.5, c='green')   #humedad    

        #etiqueta
        plt.ylabel('Temp/humedad')
        plt.xticks(orden,etiqueta, size = 'small', color = 'b') #si comentamos esta linea aparecen como etiqueta el numero de muestra
        limite_minimo = min(minimos_maximos[0][0],minimos_maximos[1][0])
        limite_maximo = max(minimos_maximos[0][1],minimos_maximos[1][1])
        plt.ylim(limite_minimo - 10, limite_maximo + 10)
        ...
        #representacion del dato 1
        plt.plot(puntos_eje_x, datos_por_sensor_y[2], lw=1.5, c='magenta')  #ph
        ...
        plt.ylabel('CO2')
        plt.xticks(orden,etiqueta, size = 'small', color = 'b') #si comentamos esta linea aparecen como etiqueta el numero de muestra
        plt.ylim(minimos_maximos[3][0] - 50, minimos_maximos[3][1] + 50)
        ...
        #       -- grafica datos 5 y 6 --
        # //-----------------------------\\
        ax = fig.add_subplot(414) #321(si es para seis graficas
        #plt.grid(True)
        plt.grid(True, lw = 0.5, ls = '--', c = '.75')

        #representacion del dato3
        plt.plot(puntos_eje_x, datos_por_sensor_y[4], lw=1.5, c='blue')  #luz azul
        plt.plot(puntos_eje_x, datos_por_sensor_y[5], lw=1.5, c='red') #luz roja

        #etiqueta
        plt.ylabel('Luz')
        plt.xticks(orden,etiqueta, size = 'small', color = 'b') #si comentamos esta linea aparecen como etiqueta el numero de muestra
        limite_minimo = min(minimos_maximos[4][0],minimos_maximos[5][0])
        limite_maximo = max(minimos_maximos[4][1],minimos_maximos[5][1])
        #plt.ylim(minimos_maximos[2][0] - 5, minimos_maximos[2][1] + 5)
        plt.ylim(limite_minimo - 50, limite_maximo + 50)
        ...
        '   FECHA:  ' + nombre_fichero_diario + '     HORA LOCAL:  ' + ('00'+str(HORA))[-2:] + ':' +('00'+str(MINUTO))[-2:]
...
nombre_fichero_diario = reloj.fecha # ".dat" #nombre del fichero Diario tomado del dia en que se inicia el programa
...
if puertoDetectado:  
    ...
    # **** ENVIO DE MENSAJE PARA NOTIFICAR AL CLIENTE DE UN REINICIO DEL SISTEMA  ****
...
# mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
    while (True):
        ...
        try:
            #if time.time() >= minuto_adquisicion_datos +15:
            if MINUTO != minuto_adquisicion_datos:
                ...
                while muestra_datos == None:
                    ...
                    muestra_datos = consultar_Arduino(2) #si hubo fallo, damos un poco mas de tiempo
                ...
                ##comprobar que los valores dela muestra son correctos  (establecer las condiciones que sean..)
                if muestra_datos[0] !=None and muestra_datos[1] !=None and muestra_datos[2] !=None:
                    ...
                    muestra_metadatos = []
                    muestra_metadatos.append("BIO_MASTER")  # Sustituir con un ID propio de cada estacion para que los datos
                    ...
                    #si todas las tareas se realizan correctamente, se actualiza el minuto para la proxima toma de muestra/datos
                    #en caso contrario  en el proximo ciclo del programa se volvera a intentar otra toma de datos
                    minuto_adquisicion_datos = MINUTO
        ...
        try:           
            ...
            #if MINUTO != minuto_dibujar_grafica and len(lista_Datos_Experimento_Bio) > 1:
            ...
            if MINUTO != minuto_error_grafica:      #solo si ha pasado un minuto del ultimo error se notifica.
                minuto_error_grafica = MINUTO       #refresco la referencia con el minuto actual
